...
; ラベルの保持さえできれば簡単ぽいが
; ラベルの取得方法
; (1) extract-labelsで、ラベルを保持して、executeでなんとかする
; (2) extract-labelsで、ラベルがきたらそれ用の処理を作ってなんとかする
; (3) gotoで処理からラベルを逆引きできる??
; 
; (1)にしてみたが gotoでレジスタを使った所が上手く動いていない
(define true #t)
(define false #f)
      ...
      'done
       (begin
        (if (eq? (caar insts) 'label)
         (begin
          (set! current-instruction-label (cdar insts))
          (advance-pc pc)
          (execute))
         (begin
          ...
          (if trace-flag
           (print "next-instruction: " current-instruction-label " " (caar insts))
           'pass)
          (execute)))))))
    (define (dispatch message)
     ...
     (extract-labels (cdr text)
      (lambda (insts labels)
       (let ((next-inst (car text)))
        ;(if (symbol? next-inst) (print next-inst) 'pass)
        (if (symbol? next-inst)
;            (receive insts
